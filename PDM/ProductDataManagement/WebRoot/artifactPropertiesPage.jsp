	...
	<tr>
		...
		<div class="dtree" id="dtree1">
    		...
    		<script type="text/javascript">
				...
				var node = new Node(<%=arts.getId() %>,0,'<%=arts.getName() %>','ArtifactServlet?tabName=t1&flag=select&artId=<%= arts.getId() %>&artName=<%=arts.getName() %>','','','','',true);
				node._hc = true;
				...
				<div CLASS="conts" ID="t2Contents" >
					 ...
					 <table id="idData" border=1 width="550px" align="center">
    					...
    					<%
      					...
      					%>
    					<tr>
    						<td width="100px"><a href="BuildServlet?tabName=t1&flag=select&buildId=<%= build.getId() %>&buildNumber=<%=build.getNumber() %>"><%=build.getNumber()%></a></td>   	
