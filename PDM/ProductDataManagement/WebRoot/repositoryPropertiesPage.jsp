...
<html>
  ...
  <body BGCOLOR="#FAF0E6" onclick="changeTabs()" onload="init()">
	...
	<% 
		...
		Repository rep = (Repository)request.getAttribute("repository");
	%>
	<tr>
	...
	<tr>
		...
		<div class="dtree" id="dtree1">
    		...
    		<script type="text/javascript">
				...
				var node = new Node(<%=art.getId() %>,0,'<%=art.getName() %>','ArtifactServlet?tabName=t1&flag=select&artId=<%= art.getId() %>&artName=<%=art.getName() %>','','','','',true);
				node._hc = true;
				...
				<div CLASS="conts" ID="t2Contents" >
					 ...
					 <table id="idData" border=1 width="550px" align="center">
    					...
    					<%
      					...
      					%>
    					<tr>
    						<td width="100px"><a href="ArtifactServlet?tabName=t1&flag=select&artId=<%= art.getId() %>&artName=<%=art.getName() %>"><%=art.getName()%></a></td>   	
