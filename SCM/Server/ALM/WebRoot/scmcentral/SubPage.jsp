...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		{
			<%
			 List<String> userAuthzs = (List<String>)request.getSession().getAttribute("userAuthzs");
			 List<String> groupAuthzs = (List<String>)request.getSession().getAttribute("groupAuthzs");
			%>
			...
			window.showModalDialog("modifyAuthz.jsp", window, "dialogWidth=400px;dialogHeight=400px;status=no");
		}
	</script>
  </head>
  ...
  <body>
	...
	<div id="Constable" class="show" style="width:90%;height=90%;overflow:scroll;">
		<table>
			<c:forEach var="log" items="${sessionScope.logMessage}">	
	...
	<div id="Vincent" class="hide" style="width:90%;height:90%;overflow:scroll;">
		<table>
			<c:forEach var="status" items="${sessionScope.statusMessage}">	
			    <tr style="font-size:14;">
			      <td bgcolor="#FFFFFF">
	...
	<div id="Monet" class="hide" style="width:90%;height=90%;">
		...
		<p>permission of current path: <c:out value="${sessionScope.currentPath}"/></p>
		...
		<table border="1">
			...
			<c:forEach var="user" items="${sessionScope.userAuthzs}">	
			    <tr style="font-size:14;">
			      <td bgcolor="#FFFFFF">
			...
			<c:forEach var="group" items="${sessionScope.groupAuthzs}">	
			    <tr style="font-size:14;">
			      <td bgcolor="#FFFFFF">
